...
>
    ...
    <xsl:key
        name="TestAssemblyToRemove"
        match="wix:Component[ substring( wix:File/@Source, string-length( wix:File/@Source ) - 9 ) = '.Tests.dll' ]"
        use="@Id"
    />
    ...
    <xsl:template match="*[ self::wix:Component or self::wix:ComponentRef ][ key( 'TestAssemblyToRemove', @Id ) ]" />
